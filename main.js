...
game.score = 0;
game.playerName = '';
game.playerName = window.prompt('Player\'s name -- maximum of 12 characters: ');
if (game.playerName == ''){
        game.playerName = 'anonymous';
}
while (game.playerName.length > 12) {
        game.playerName = window.prompt('Please, try again with a maximum of 12 characters: ');
        continue;
}
...
// players name
var drawPlayerNameOnCanvas = game.canvas.getContext('2d');

function drawPlayerName() {
        var fixedPlayerText = 'Player: ';
        var textSize = (game.playerName.length + fixedPlayerText.length) * 10;  
        drawPlayerNameOnCanvas.font = 'bold 18px Mono'; // monospace - all letters have equal sizes 
        drawPlayerNameOnCanvas.fillStyle = 'Gray';
        drawPlayerNameOnCanvas.fillText(fixedPlayerText + game.playerName, 780-textSize, 20); 
}       
